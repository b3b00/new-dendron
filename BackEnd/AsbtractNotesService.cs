...
namespace BackEnd
{
    ...
    public abstract class AsbtractNotesService : INotesService
    {
        ...
        public abstract bool IsFileSystemRepo();
        ...
        public async Task<Result<Dendron>> GetDendron(bool includeContent = true)
        {
            ...
            if (includeContent)
            {
                var tasks = noteNames.TheResult.Select(noteName => GetNote(noteName)).ToList();
                var results = await Task.WhenAll(tasks);

                foreach (var noteResult in results)
                {
                    if (!noteResult.IsOk)
                    {
                        return Result<Dendron>.TransformError<Note, Dendron>(noteResult);
                    }
                    dendron.Put(noteResult.TheResult);
                }
            }
    ...
    public virtual Task<List<(string name, long id)>> GetRepositories()
    {
      throw new System.NotImplementedException();
    }

    public virtual Task<Stream> GetImageStream(string fileName)
    {
      throw new System.NotImplementedException();
    }
  }
